...
os.makedirs('corpus_use', exist_ok=True)
def random_sample_two_files(input_file1, input_file2, output_file1, output_file2, num_lines=50):
    """从两个文件中同步随机抽取相同行号的内容"""
    
    # 读取第一个文件的所有行
    with open(input_file1, 'r', encoding='GBK') as f:
        lines1 = f.readlines()
    
    # 读取第二个文件的所有行
    with open(input_file2, 'r', encoding='GBK') as f:
        lines2 = f.readlines()
    ...
    # 保存到输出文件1
    with open(output_file1, 'w', encoding='GBK') as f:
        f.writelines(sampled_lines1)
    
    # 保存到输出文件2
    with open(output_file2, 'w', encoding='GBK') as f:
...
if __name__ == "__main__":
    # 输入文件
    input_file1 = "corpus_cleaned/cleaned.corpus.txt"
    input_file2 = "corpus_cleaned/sample_corpus.txt"
    
    # 输出文件
    output_file1 = "corpus_use/50_lines_test.txt"
    output_file2 = "corpus_use/50_lines_sampled.txt"
