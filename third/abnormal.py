...
def save_to_gbk(filename, sentences):
    try:
        # 强制用 gbk 保存，errors='replace' 把不能编码的字变成问号，防止报错
        with open(filename, 'w', encoding='gbk', errors='replace') as f:
            for s in sentences:
                f.write(s + '\n')
        print(f"? 文件已保存: {filename} (GBK, {len(sentences)} 行)")
...
def crawl_jokes():
    ...
    save_to_gbk('corpus_jokes_gbk.txt', sentences[:TARGET_COUNT])
...
def crawl_tieba():
    ...
    save_to_gbk('corpus_tieba_gbk.txt', sentences[:TARGET_COUNT])
...
def crawl_zhihu():
    ...
    save_to_gbk('corpus_zhihu_gbk.txt', sentences[:TARGET_COUNT])
...
def crawl_weibo():
    ...
    save_to_gbk('corpus_weibo_gbk.txt', sentences[:TARGET_COUNT])
...
def crawl_douban():
    ...
    save_to_gbk('corpus_douban_gbk.txt', sentences[:TARGET_COUNT])
...
def crawl_lifestyle():
    ...
    save_to_gbk('corpus_lifestyle_gbk.txt', sentences[:TARGET_COUNT])
