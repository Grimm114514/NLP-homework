...
\begin{enumerate}
    ...
    \begin{itemize}
        \item 爬虫执行如下方案：\textbf{仅提取当前页面的论文摘要（Abstract）文本}。摘要文本位于 \texttt{<blockquote>} 标签中。
    \end{itemize}

    \item \textbf{内容提取 (\texttt{parse\_html\_paper} / \texttt{parse\_abstract}):} 如果执行优先策略，爬虫会进入 \texttt{parse\_html\_paper} 方法，提取全文内容；如果执行备选策略，则在 \texttt{parse\_abstract} 方法中提取摘要内容。
\end{enumerate}

\subsection{新浪新闻数据爬取 (sina\_tech.py)}

\texttt{sina\_tech.py} 爬虫的逻辑相对直接。它从首页开始，在 \texttt{parse} 方法中使用 XPath (\texttt{response.xpath}) 筛选出所有符合新浪文章 URL 特征（包含 \texttt{/doc-} 和 \texttt{.shtml}）的链接。随后，它跟进这些链接，并在 \texttt{parse\_article} 方法中，使用 XPath 精确提取文章的\textbf{标题、发布时间、来源}和\textbf{正文}（\texttt{div\#artibody}）等结构化数据。

\subsection{核心爬取配置 (settings.py)}

为了确保爬取过程的高效、稳定和友好，我们在 \texttt{settings.py} 文件中进行如下参数设置：

\begin{enumerate}
    \item \textbf{友好性策略：} 为了避免对目标服务器造成过大压力，我们组合使用了 \texttt{ROBOTSTXT\_OBEY = True}（严格遵守网站的 \texttt{robots.txt} 爬取协议）和 \texttt{DOWNLOAD\_DELAY = 1}（设置 1 秒的下载延迟）。
    \item \textbf{身份伪装：} 我们将 \texttt{USER\_AGENT} 设置为通用的 Chrome 浏览器标识，以模拟正常的用户访问，这是爬虫反屏蔽的常见策略。
    \item \textbf{数据编码：} 鉴于新浪新闻包含大量中文文本，我们明确设置 \texttt{FEED\_EXPORT\_ENCODING = "utf-8"}。
\end{enumerate}
\section{样本的清洗}
...
清洗步骤如下：
\begin{enumerate}
    \item \textbf{去除 HTML 标签：} 使用 BeautifulSoup 的 \texttt{get\_text()} 方法提取所有文本。
    \item \textbf{去除脚本和样式：} 剔除 \texttt{<script>} 和 \texttt{<style>} 标签内的内容。
    \item \textbf{去除无效字符：} 使用正则表达式去除多余的空白符、换行符和特殊控制字符。
    \item \textbf{统一编码：} 所有文本统一保存为 UTF-8 编码。
\end{enumerate}

最终，我们得到了三个不同规模的中文和英文语料库：
\begin{itemize}
    \item \textbf{小规模样本：} 约 1MB 文本。
    \item \textbf{中规模样本：} 约 10MB 文本。
    \item \textbf{大规模样本：} 约 100MB 文本。
\end{itemize}
